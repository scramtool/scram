...
from 
  (select report.task_id, task.description, task.status, max( date) as end, min(date) as start, sum( burnt) as total_burnt from report, task where task.task_id = report.task_id and task.sprint_id =2 and task.status != 'forwarded' group by report.task_id) as task_info, 
